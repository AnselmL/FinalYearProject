...
for fold = 1:nb_folds
    ...
    while(~isempty(TIMIT_wav_pool))
        ...
        if isempty(hrtf_type_pool)
            hrtf_type_pool = 1:length(hrtf_type);
        end
        ...
        if ~any(alpha_vals >= 1)
            ...
            elseif radius_target >= 1.45 && radius_target < 2.7
                hrtf_subdir = 'MMHR-HRTF\BKwHA';
                radius_hrtf = 1.7;
            elseif radius_target >= 2.7
                hrtf_subdir = 'FF_KOELN';
                radius_hrtf = 3.25;
            else
            ...
            switch hrtf_subdir
                ...
                otherwise
                    hrtf_el = el_target(el_index);
            end
                   
                    
            hrtf_dir = strcat('az_',num2str(az_target(az_index)),'_el_',num2str(hrtf_el),'_radius_',num2str(radius_hrtf));

            hrtf_out_dir = strcat(HRTF_base_dir,'\',hrtf_subdir,'\',hrtf_dir,'.wav');

            [HRIR,fs_HRIR] = audioread(hrtf_out_dir);
            ...
            hrtf_index = hrtf_type_pool(randperm(size(hrtf_type_pool,2),1));
            hrtf_type_pool=hrtf_type_pool(hrtf_type_pool~=hrtf_index);
            ...
            hrtf_subdir_dir = strcat(HRTF_base_dir,'\',hrtf_subdir);
            file_type = '.wav';
            wav_files = dir(strcat(hrtf_subdir_dir,'\','*',file_type));
            wav_files = {wav_files.name};
            nb_files = length(wav_files);

            rand_file_ind = randi(nb_files);

            hrtf_out_dir = strcat(hrtf_subdir_dir,'\',wav_files{rand_file_ind});
            [HRIR,fs_HRIR] = audioread(hrtf_out_dir);
            if size(HRIR,1) < size(HRIR,2)
                HRIR = HRIR.';
            end
            ...
            BRIR = HRIR;
            %pick random impulse response from directory

            %set impulse response = hrtf
        end
            ...
            if ~any(alpha_vals >= 1)
                ...
                switch hrtf_subdir
                    case 'SCUT'
                        ...
                        radius_hrtf_direc = round(radius_target_direc(1),1);
                        if radius_hrtf_direc > 1
                            radius_hrtf_direc = 1;
                        end
                    case 'ARI_bte'
                        radius_increment = 0.05*max_radius_target;
                        ...
                        considered_index = randi(length(radii_considered));
                        radius_target_direc(1) = radii_considered(considered_index);
                        radius_hrtf_direc = 1.2;
                        hrtf_el_direc(1) = el_target(el_index_direc(1));
                ...
                hrtf_dir_direc{1} = strcat('az_',num2str(az_target(az_index_direc(1))),'_el_',num2str(hrtf_el_direc(1)),'_radius_',num2str(radius_hrtf_direc(1)));

                hrtf_out_dir_direc{1} = strcat(HRTF_base_dir,'\',hrtf_subdir,'\',hrtf_dir_direc{1},'.wav');

                [HRIR_direc{1},fs_HRIR_direc(1)] = audioread(hrtf_out_dir_direc{1});
                ...
                if direc_on(direc_index) == 2
                    ...
                    switch hrtf_subdir
                        ...
                        otherwise
                            hrtf_el_direc(2) = el_target(el_index_direc(2));
                    end
                    ...
                    hrtf_dir_direc{2} = strcat('az_',num2str(az_target(az_index_direc(2))),'_el_',num2str(hrtf_el_direc(2)),'_radius_',num2str(radius_hrtf_direc));

                    hrtf_out_dir_direc{2} = strcat(HRTF_base_dir,'\',hrtf_subdir,'\',hrtf_dir_direc{2},'.wav');

                    [HRIR_direc{2},fs_HRIR_direc(2)] = audioread(hrtf_out_dir_direc{2});
                ...
                hrtf_subdir_direc_dir = strcat(HRTF_base_dir,'\',hrtf_subdir);
                file_type = '.wav';
                wav_files = dir(strcat(hrtf_subdir_dir,'\','*',file_type));
                wav_files = {wav_files.name};
                nb_files = length(wav_files);
                ...
                hrtf_out_dir_direc{1} = strcat(hrtf_subdir_direc_dir,'\',wav_files{rand_file_ind(1)});
                [HRIR_direc{1},fs_HRIR_direc] = audioread(hrtf_out_dir_direc{1});

                HRIR_direc{1} = resample(HRIR_direc{1},fs_speech,fs_HRIR_direc);

                BRIR_direc{1} = HRIR_direc{1};
                if direc_on(direc_index) == 2
                    hrtf_out_dir_direc{2} = strcat(hrtf_subdir_direc_dir,'\',wav_files{rand_file_ind(2)});
                    [HRIR_direc{2},fs_HRIR_direc] = audioread(hrtf_out_dir_direc{2});
