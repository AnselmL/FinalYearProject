...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/stft_nonintrusive/'
...
def nparray_to_tfrecord(mat_dir_feat, subdirs, tfrecrod_fname):
    ...
    def load_mat_features(fname_mag, fname_phase, fname_H):
        ...
        X = np.empty((D_zp.shape[0], D_zp.shape[1], 1), dtype=D_zp.dtype)
...
def tfrecord_to_dataset(nfeatures, nb_frames, tfrecord_fname, batch_size, shuffle_buffer):
    ...
    def _parse_function(proto):
        ...
        keys_to_features = { 'data': tf.io.FixedLenFeature([nfeatures,nb_frames,1], tf.float32),\
        ...
        parsed_features['data'] = tf.reshape( tf.reshape( parsed_features['data'], (1,nfeatures,nb_frames,1))[:,:,:,:], (nfeatures,nb_frames,1))
