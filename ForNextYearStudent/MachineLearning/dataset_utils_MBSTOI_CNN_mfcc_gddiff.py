...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/mfcc_gddiff/'
...
def mat_to_nparray_test(mat_dir_feat_test):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/mfcc_gddiff/'
    if (not (os.path.isdir(tfrecord_dir))):
        os.makedirs(tfrecord_dir)
...
def nparray_to_tfrecord(mat_dir_feat, subdirs, tfrecrod_fname):
    ...
    def load_mat_features(fname_mag, fname_phase):
        ...
        c_phase = cl_phase - cr_phase
        d_phase = dl_phase - dr_phase

        C = np.empty((2 * cl_mag.shape[0] + c_phase.shape[0], cl_mag.shape[1]), dtype=cl_mag.dtype)

        C[0:(cl_mag.shape[0]), :] = cl_mag
        C[(cl_mag.shape[0]):(2*cl_mag.shape[0]),:] = cr_mag
        C[(2*cl_mag.shape[0]):(2*cl_mag.shape[0] + c_phase.shape[0]), :] = c_phase
        ...
        D = np.empty((2*dl_mag.shape[0] + d_phase.shape[0], dl_mag.shape[1]), dtype=dl_mag.dtype)

        D[0:(dl_mag.shape[0]), :] = dl_mag
        D[(dl_mag.shape[0]):(2*dl_mag.shape[0]),:] = dr_mag
        D[(2*dl_mag.shape[0]):(2*dl_mag.shape[0] + d_phase.shape[0]), :] = d_phase
    ...
    X = load_mat_features(mat_fname_features_mag, mat_fname_features_phase)
