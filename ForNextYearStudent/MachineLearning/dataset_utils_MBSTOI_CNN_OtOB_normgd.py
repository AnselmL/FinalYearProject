...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/OtOB_normgd/'
...
def nparray_to_tfrecord(mat_dir_feat, subdirs, tfrecrod_fname):
    ...
    def load_mat_features(fname_mag, fname_phase, fname_H):
        ...
        for i in range(1,cl_phase.shape[1]-1):
        ...
        cl_phase = np.matmul(H, cl_phase)
        cr_phase = np.matmul(H, cr_phase)
        dl_phase = np.matmul(H, dl_phase)
        dr_phase = np.matmul(H, dr_phase)
