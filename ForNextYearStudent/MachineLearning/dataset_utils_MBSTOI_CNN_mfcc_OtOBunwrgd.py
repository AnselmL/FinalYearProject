...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/mfcc_OtOBunwrgd/'
...
def mat_to_nparray_test(mat_dir_feat_test):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/mfcc_OtOBunwrgd/'
    if (not (os.path.isdir(tfrecord_dir))):
        os.makedirs(tfrecord_dir)
...
def nparray_to_tfrecord(mat_dir_feat, subdirs, tfrecrod_fname):
    ...
    def load_mat_features(fname_mag, fname_phase, fname_H):
        ...
        cl_phase = np.unwrap(cl_phase)
        cr_phase = np.unwrap(cr_phase)
        dl_phase = np.unwrap(dl_phase)
        dr_phase = np.unwrap(dr_phase)
