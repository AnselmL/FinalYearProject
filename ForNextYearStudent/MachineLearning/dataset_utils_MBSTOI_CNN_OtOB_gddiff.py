...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/OtOB_gddiff/'
...
def nparray_to_tfrecord(mat_dir_feat, subdirs, tfrecrod_fname):
    ...
    def load_mat_features(fname_mag, fname_phase, fname_H):
        ...
        C = np.empty((cl_mag.shape[0] + cl_mag.shape[0] + c_phase.shape[0], cl_mag.shape[1]), dtype=cl_mag.dtype)
        C[0::3, :] = cl_mag
        C[1::3, :] = c_phase
        C[2::3, :] = cr_mag
        ...
        D = np.empty((dl_mag.shape[0] + dl_mag.shape[0] + d_phase.shape[0], dl_mag.shape[1]), dtype=dl_mag.dtype)
        D[0::3, :] = dl_mag
        D[1::3, :] = d_phase
        D[2::3, :] = dr_mag
