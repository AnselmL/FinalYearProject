...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    subdirs = [subdir for subdir in subdirs_pre if int(subdir) < 60000]
    # # subdirs_data= {
    # #     'subdirs': subdirs
    # # }
    ...
    # val_idx = val_idx - 1
    if not(os.path.isfile('/home/al5517/ML_data/directional_2021_06_02/labels_val_recon_online.mat')):
        val_size = 0.2
        val_idx = np.random.choice(len(subdirs), int(np.around(len(subdirs) * val_size)), replace=False)
    else:
        mat_contents_val_idx = sio.loadmat('/home/al5517/ML_data/directional_2021_06_02/labels_val_recon_online.mat')
        val_idx = mat_contents_val_idx['labels_val_recon']
        val_idx = val_idx.flatten()
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/stft_online/'
...
def mat_to_nparray_test(mat_dir_feat_test):
    ...
    # Pre-allocate table, ready for data:
    test_subdirs_pre = get_subdirs(mat_dir_feat_test)
    test_subdirs = [subdir for subdir in test_subdirs_pre if int(subdir) < 30000]
...
def nparray_to_tfrecord(mat_dir_feat, subdirs, tfrecrod_fname):
    ...
    def load_mat_features(fname_mag, fname_phase, fname_H):
        ...
        C = np.empty((cl_mag.shape[0] + cl_mag.shape[0] + cl_mag.shape[0] + cl_mag.shape[0], cl_mag.shape[1]), dtype=cl_mag.dtype)
        C[0::4, :] = cl_mag
        C[1::4, :] = cl_phase
        C[2::4, :] = cr_mag
        C[3::4, :] = cr_phase
        ...
        D = np.empty((dl_mag.shape[0] + dl_mag.shape[0] + dr_mag.shape[0] + dr_mag.shape[0], dl_mag.shape[1]), dtype=dl_mag.dtype)
        D[0::4, :] = dl_mag
        D[1::4, :] = dl_phase
        D[2::4, :] = dr_mag
        D[3::4, :] = dr_phase
        #print(f'Distorted features dim {D.shape}')
    ...
    mat_fname_H = '/home/al5517/feature_preprocessing_output/directional_2021_06_02/TRAIN/H_trunc.mat'
