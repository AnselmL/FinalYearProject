...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/mfcc_OtOBnormphase/'
...
def mat_to_nparray_test(mat_dir_feat_test):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/mfcc_OtOBnormphase/'
    if (not (os.path.isdir(tfrecord_dir))):
        os.makedirs(tfrecord_dir)
...
def nparray_to_tfrecord(mat_dir_feat, subdirs, tfrecrod_fname):
    ...
    def load_mat_features(fname_mag, fname_phase, fname_H):
        ...
        cl_mag = mat_contents_mag['xl_mfcc']
        cl_phase = mat_contents_phase['xl_hat_angle']
        cr_mag = mat_contents_mag['xr_mfcc']
        cr_phase = mat_contents_phase['xr_hat_angle']
        dl_mag = mat_contents_mag['yl_mfcc']
        dl_phase = mat_contents_phase['yl_hat_angle']
        dr_mag = mat_contents_mag['yr_mfcc']
        dr_phase = mat_contents_phase['yr_hat_angle']
        H = mat_contents_H['H_trunc']

        cl_mag = np.transpose(cl_mag)
        cr_mag = np.transpose(cr_mag)
        dl_mag = np.transpose(dl_mag)
        dr_mag = np.transpose(dr_mag)

        for i in range(0,cl_phase.shape[1]):
            cl_phase[:,i] = cl_phase[:,i]/cl_phase[23,i]
            cr_phase[:, i] = cr_phase[:, i] / cr_phase[23, i]
            dl_phase[:, i] = dl_phase[:, i] / dl_phase[23, i]
            dr_phase[:, i] = dr_phase[:, i] / dr_phase[23, i]

        for i in range(0, H.shape[0]):
            H[i, :] = H[i, :]/np.count_nonzero(H[i, :] == 1)

        cl_OtOB_phase = np.matmul(H, cl_phase)
        cr_OtOB_phase = np.matmul(H, cr_phase)
        dl_OtOB_phase = np.matmul(H, dl_phase)
        dr_OtOB_phase = np.matmul(H, dr_phase)


        C = np.empty((2 * cl_mag.shape[0] + 2 * cl_OtOB_phase.shape[0], cl_mag.shape[1]), dtype=cl_mag.dtype)
        C[0:(cl_mag.shape[0]), :] = cl_mag
        C[(cl_mag.shape[0]):(2*cl_mag.shape[0]),:] = cr_mag
        C[(2*cl_mag.shape[0]):(2*cl_mag.shape[0] + cl_OtOB_phase.shape[0]), :] = cl_OtOB_phase
        C[(2*cl_mag.shape[0] + cl_OtOB_phase.shape[0]):(2*cl_mag.shape[0] + 2*cl_OtOB_phase.shape[0]), :] = cr_OtOB_phase
        ...
        D = np.empty((2*dl_mag.shape[0] + 2*dl_OtOB_phase.shape[0], dl_mag.shape[1]), dtype=dl_mag.dtype)

        D[0:(dl_mag.shape[0]), :] = dl_mag
        D[(dl_mag.shape[0]):(2*dl_mag.shape[0]),:] = dr_mag
        D[(2*dl_mag.shape[0]):(2*dl_mag.shape[0] + dl_OtOB_phase.shape[0]), :] = dl_OtOB_phase
        D[(2*dl_mag.shape[0] + dl_OtOB_phase.shape[0]):(2*dl_mag.shape[0] + 2*dl_OtOB_phase.shape[0]), :] = dr_OtOB_phase
    ...
    #total_frames = 0
    mat_fname_features_mag = mat_dir_feat + subdirs[0] + '/mfcc.mat'
    ...
    for idx in range(len(subdirs)):
        mat_fname_features_mag = mat_dir_feat + subdirs[idx] + '/mfcc.mat'
