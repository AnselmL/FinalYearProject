...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/mfcc_OtOBphasediff/'
...
def mat_to_nparray_test(mat_dir_feat_test):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/mfcc_OtOBphasediff/'
    if (not (os.path.isdir(tfrecord_dir))):
        os.makedirs(tfrecord_dir)
...
def nparray_to_tfrecord(mat_dir_feat, subdirs, tfrecrod_fname):
    ...
    def load_mat_features(fname_mag, fname_phase, fname_H):
        ...
        c_OtOB_phase = cl_OtOB_phase - cr_OtOB_phase
        d_OtOB_phase = dl_OtOB_phase - dr_OtOB_phase

        C = np.empty((2 * cl_mag.shape[0] + c_OtOB_phase.shape[0], cl_mag.shape[1]), dtype=cl_mag.dtype)
        C[0:(cl_mag.shape[0]), :] = cl_mag
        C[(cl_mag.shape[0]):(2*cl_mag.shape[0]),:] = cr_mag
        C[(2*cl_mag.shape[0]):(2*cl_mag.shape[0] + c_OtOB_phase.shape[0]), :] = c_OtOB_phase
        ...
        D = np.empty((2*dl_mag.shape[0] + d_OtOB_phase.shape[0], dl_mag.shape[1]), dtype=dl_mag.dtype)

        D[0:(dl_mag.shape[0]), :] = dl_mag
        D[(dl_mag.shape[0]):(2*dl_mag.shape[0]),:] = dr_mag
        D[(2*dl_mag.shape[0]):(2*dl_mag.shape[0] + d_OtOB_phase.shape[0]), :] = d_OtOB_phase
