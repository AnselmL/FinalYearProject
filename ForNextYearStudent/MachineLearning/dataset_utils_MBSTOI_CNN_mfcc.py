...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/mfcc/'
...
def mat_to_nparray_test(mat_dir_feat_test):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/mfcc/'
    if (not (os.path.isdir(tfrecord_dir))):
        os.makedirs(tfrecord_dir)
...
def nparray_to_tfrecord(mat_dir_feat, subdirs, tfrecrod_fname):
    ...
    def load_mat_features(fname):
        """Load data and labels from a single mat file"""

        # print(sio.whosmat(fname))
        mat_contents = sio.loadmat(fname)
        cl_mag = mat_contents['xl_mfcc']
        #cl_phase = mat_contents['xl_lf_phase_norm']
        cr_mag = mat_contents['xr_mfcc']
        #cr_phase = mat_contents['xr_lf_phase_norm']
        dl_mag = mat_contents['yl_mfcc']
        #dl_phase = mat_contents['yl_lf_phase_norm']
        dr_mag = mat_contents['yr_mfcc']
        #dr_phase = mat_contents['yr_lf_phase_norm']
        ...
        C = np.empty((cl_mag.shape[0] + cr_mag.shape[0], cl_mag.shape[1]), dtype=cl_mag.dtype)



        C[0:(cl_mag.shape[0]), :] = cl_mag
        C[(cl_mag.shape[0]):(cl_mag.shape[0]+ cr_mag.shape[0]),:] = cr_mag
        ...
        D = np.empty((dl_mag.shape[0] + dr_mag.shape[0], dl_mag.shape[1]), dtype=dl_mag.dtype)



        D[0:(dl_mag.shape[0]), :] = dl_mag
        D[(dl_mag.shape[0]):(dl_mag.shape[0] + dr_mag.shape[0]), :] = dr_mag
    ...
    #total_frames = 0
    mat_fname_features = mat_dir_feat + subdirs[0] + '/mfcc.mat'
    X = load_mat_features(mat_fname_features)
    ...
    for idx in range(len(subdirs)):
        mat_fname_features = mat_dir_feat + subdirs[idx] + '/mfcc.mat'
