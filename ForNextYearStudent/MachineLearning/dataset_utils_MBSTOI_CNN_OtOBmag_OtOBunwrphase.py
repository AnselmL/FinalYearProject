...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/OtOBmag_OtOBunwrphase/'
...
def mat_to_nparray_test(mat_dir_feat_test):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/OtOBmag_OtOBunwrphase/'
    if (not (os.path.isdir(tfrecord_dir))):
        os.makedirs(tfrecord_dir)
...
def nparray_to_tfrecord(mat_dir_feat, subdirs, tfrecrod_fname):
    ...
    def load_mat_features(fname_mag, fname_phase, fname_H):
        ...
        cl_mag_temp = np.matmul(H, np.square(cl_mag))
        cr_mag_temp = np.matmul(H, np.square(cr_mag))
        dl_mag_temp = np.matmul(H, np.square(dl_mag))
        dr_mag_temp = np.matmul(H, np.square(dr_mag))
        ...
        #print(f' cl_mag shape: {cl_mag.shape}')
        #print(f' cl_OtOB_phase shape: {cl_OtOB_phase.shape}')
