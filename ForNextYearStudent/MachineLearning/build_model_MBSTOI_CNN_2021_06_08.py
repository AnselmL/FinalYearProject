...
def build_model(optimizer, loss, metrics, nFilters1, KernelSize1, input_dim1, input_dim2, act_func,dropout_val, fc_1_neurons, fc_2_neurons):
    ...
    if act_func == 'elu':
        model.add(tf.keras.layers.Dense(fc_1_neurons, activation='elu'))
    elif act_func == 'tanh':
        model.add(tf.keras.layers.Dense(fc_1_neurons, activation='tanh'))
    elif act_func == 'selu':
        model.add(tf.keras.layers.Dense(fc_1_neurons, activation='selu'))
    elif act_func == 'none':
    ...
    if act_func == 'elu':
        model.add(tf.keras.layers.Dense(fc_2_neurons, activation='elu'))
    elif act_func == 'tanh':
        model.add(tf.keras.layers.Dense(fc_2_neurons, activation='tanh'))
    elif act_func == 'selu':
        model.add(tf.keras.layers.Dense(fc_2_neurons, activation='selu'))
    elif act_func == 'none':
