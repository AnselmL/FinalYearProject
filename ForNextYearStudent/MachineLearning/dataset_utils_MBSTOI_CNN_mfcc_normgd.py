...
def mat_to_tfrecord(mat_dir_feat_train):
    ...
    tfrecord_dir = '/home/al5517/ML_data/directional_2021_06_02/tfrecord/mfcc_normgd/'
...
def nparray_to_tfrecord(mat_dir_feat, mat_dir_lbl, subdirs, tfrecrod_fname):
    """
    ...
    for idx in range(len(subdirs)):
        ...
        mat_fname_label = mat_dir_lbl + subdirs[idx] + '/mbstoi.mat'
...
def gen_nparray(mat_dir_feat, mat_dir_lbl, subdirs):
    """
    Loads mat file to nparray and writes the array to tfrecord (standard format for datasets in tensorflow)
    """
    ...
    def load_mat_features(fname_mag, fname_phase):
        ...
        for i in range(0, cl_phase.shape[1] - 1):
            cl_phase[:, cl_phase.shape[1] - i] = cl_phase[:, cl_phase.shape[1] - i] - cl_phase[:,
                                                                                      cl_phase.shape[1] - (i + 1)]
            cr_phase[:, cr_phase.shape[1] - i] = cr_phase[:, cr_phase.shape[1] - i] - cr_phase[:,
                                                                                      cr_phase.shape[1] - (i + 1)]
            dl_phase[:, dl_phase.shape[1] - i] = dl_phase[:, dl_phase.shape[1] - i] - dl_phase[:,
                                                                                      dl_phase.shape[1] - (i + 1)]
